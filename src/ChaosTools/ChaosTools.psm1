...
function Invoke-ChaosTest {
    <#
    .SYNOPSIS
        Executes a script block with random failures injected.

    .DESCRIPTION
        Wraps the specified commands and randomly delays execution,
        throws transient errors or tweaks parameter values. All
        chaos events are written to the log using Write-STLog so
        the effects can be reviewed later.

    .PARAMETER ScriptBlock
        The commands to execute while chaos testing is enabled.

    .PARAMETER Scope
        Names of commands that should be intercepted for injection.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
