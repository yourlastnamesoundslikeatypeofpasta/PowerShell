    ...
    Context 'Set-SharedMailboxAutoReply behavior' {
        It 'defaults ExternalMessage to InternalMessage' {
            InModuleScope SupportTools {
                ...
                Assert-MockCalled Set-MailboxAutoReplyConfiguration -ParameterFilter { $ExternalMessage -eq 'hello' } -Times 1 -ModuleName SupportTools
        ...
        It 'uses web login when specified' {
            InModuleScope SupportTools {
                ...
                Assert-MockCalled Connect-ExchangeOnline -ParameterFilter { $UseWebLogin } -Times 1 -ModuleName SupportTools
            }
        }
    }
