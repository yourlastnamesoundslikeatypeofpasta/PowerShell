    ...
    Context 'Get-SPToolsLibraryReport' {
        It 'returns library data' {
            InModuleScope SharePointTools {
                ...
                $script:lists = @(
                    [pscustomobject]@{ Title='Docs'; ItemCount=1; LastItemUserModifiedDate=[datetime]'2024-01-01' },
                    [pscustomobject]@{ Title='Records'; ItemCount=2; LastItemUserModifiedDate=[datetime]'2024-02-01' }
                )
    ...
    Context 'Clear-SPToolsRecycleBin' {
        It 'clears first stage by default' {
            InModuleScope SharePointTools {
                ...
                Assert-MockCalled Clear-PnPRecycleBinItem -Times 1 -ParameterFilter { $FirstStage -and $Force }
            }
        }
        It 'clears second stage when requested' {
            InModuleScope SharePointTools {
                ...
                Assert-MockCalled Clear-PnPRecycleBinItem -Times 1 -ParameterFilter { $SecondStage -and $Force }
            }
        }
    }
